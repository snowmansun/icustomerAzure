...
router.get('/info', function (req, res) {
    ...
    var query = 'select a.accountnumber customercode,c.name customername,c.accountid ' +
        ...
        ' where ebmobile__primary__c= 1 and a.accountnumber = \'' + req.query.accountnumber + '\'';
...
router.post('/update', function (req, res) {
    var query = 'update contact set LocalModified=1,ebMobile__GUID__c=isnull(ebMobile__GUID__c,newid()),firstname=\'' + req.body.firstname + '\',lastname=\'' + req.body.lastname + '\',email=\'' + req.body.email + '\',mobilephone=\'' + req.body.mobile + '\' where accountid=\'' + req.body.accountid + '\' and ebmobile__primary__c= 1 ';
    dbHelper.query(query, function (err, result) {
        if (err) {
